...
@tool
def skill_map_bg_bool(user_id: str, thread_id: str, skill: str) -> bool:
    """Trigger background skill map generation. Returns True when started."""
    try:
        scheduler.add_job(
            skill_split_bg,
            args=[user_id, thread_id, skill],
...
def progress_tracker(state: AgentState) -> dict[str, Any] | None:
    ...
    # Get current_topic to identify which depth=1 node we're in
    ...
    Just because the LLM has mentioned a topic, doesnt mean the user has learned it. IT MUST BE TAUGHT. For a particular depth=0, if the llm taught only 2 contents inside it, you must update the progress value for only those 2 contents. NOT THE REMAINING SINCE THAT IS NOT YET TAUGHT TO THE USER. YOU MUST ALWAYS DO THIS. UPDATE ONLY WHAT IS TAUGHT, just because a particular depth=0 is asked by the user, doesnt mean the llm has taught all those topics to update them. ONLY A FEW ARE TAUGHT TO THE USER AND TRACE ONLY THOSE AND UPDATE THOSE THAT'S IT.

    OUTPUT FORMAT - CRITICAL:
    You MUST return a JSON list (array) where each element is a dictionary with ONE content heading and its progress.
    
    EXAMPLES:
    
    Single topic:
    [{{"Lists and List Comprehensions": {{"progress": 10}}}}]
    
    Multiple topics:
    [{{"Lists and List Comprehensions": {{"progress": 10}}}}, {{"Dictionaries": {{"progress": 10}}}}]
    
    No learning detected:
    null

    If there is already a certain progress value for a certain heading, you can increment the progress by 10 points:
    Current progress:
    [{{"Lists and List Comprehensions": {{"progress": 10}}}}]
    New progress (if you detect the user learning this topic again):
    [{{"Lists and List Comprehensions": {{"progress": 20}}}}]

    UNBREAKABLE RULE:
    YOU CAN ONLY UPDATE THE PROGRESS OF THE CONTENTS INSIDE THE DEPTH=0 NODES. NOT THE HIGHER LEVEL ONES.

    RULES:
    1. Under each depth=0 node in the skill map, there is a contents dictionary with specific headings. You must ONLY identify these exact headings. Even if the user wants to learn about any of the higher level topics, like ones with depth=0 and above, you must identify the content headings of the depth=0 nodes ONLY. Inside the depth=0 nodes', contents are the only ones that you must identify. Not the higher level ones. If the user mentioned a higher level topic, you cant update that topic, you must trace down to the contents inside the depth=0 nodes only and identify what matches and only that you are ALLOWED TO UPDATE. If any of the higher level topics are mentioned, ie, anything with depth>=0, you SHOULD trace it down to what contents inside a depth=0 matches ONLY, THEN you can update the progress value. You must trace it down, and only then update that progress value.
    
    IMPORTANT RULE: Only Update what has been taught by the llm not what the user is asking about. You must map the topic taught by the llm to the content headings inside the depth=0 nodes and update the progress value accordingly. You must be very robust in this case since just an overview mentioning of a topic doesnt mean the llm has taught all the contents inside it. So you should understand what has been 'TAUGHT' by the llm and update the progress value accordingly.

    IMPORTANT EXAMPLE YOU MUST LEARN FROM:
    See for a high level topic, YOU MUST NOT UPDATE ALL THE contents under it, it must be very specific wrt the content that has been taught to the user. For example in the skill map, say the current topic is Control flow, and the user question is "Control Flow", in that case YOU MUST ASSESS THE LLM RESPONSE and see what topics are being covered there. So as per the skill map, say "Control flow" has 5 contents inside it - if-else loops, switch statements, for loops , while and do-while and break and continue. You must only see what the llm has taught in its AIMessage response. So if the llm taught only if-else and for loops - UPDATE ONLY THOSE 2 content values in the progress. NOT THE REMAINING SINCE THAT IS NOT YET TAUGHT TO THE USER. YOU MUST ALWAYS DO THIS. UPDATE ONLY WHAT IS TAUGHT, just because a particular depth=0 is asked by the user, doesnt mean the llm has taught all those topics to update them. ONLY A FEW ARE TAUGHT TO THE USER AND TRACE ONLY THOSE AND UPDATE THOSE THAT"S IT.

    NOT ALL THE CONTENTS UNDER A DEPTH = 0 NODE SHOULD BE UPDATED BASIS THE USER'S QUESTION. ITS ONLY UPDATION BASIS WHAT THE LLM HAS TAUGHT. Trace it down to the contents inside the depth=0 nodes only and identify what matches and only that you are ALLOWED TO UPDATE. That's how specific you must be. So you must act like a smart teacher here who can map the learning to the topic, rather than a generic progress tracker.

    ANOTHER IMPORTANT NOTE: If an already taught with the same crux for any content inside depth=0 node, you must NOT update the progress value. Learning and Teaching must always be new. If the user is deep diving into a topic, and the llm is teaching it, you must update the progress value for that topic. But if the user is asking about a topic that has already been taught, you must NOT update the progress value. THAT'S PRECISION I'M TALKING ABOUT HERE. Since now you have context of the conversation history, you must be very vigilant in this sense. 
    
    2. Match the LLM's teaching response STRICTLY to content headings, inside the depth=0 nodes in the skill map.
    3. Return a JSON list with each heading as a separate object.
    4. Each progress increment is 10 points.
    5. You can NEVER reduce progress, only increase.
    6. If no learning detected, return null (last resort only).
    7. NO extra text, NO explanations, ONLY the JSON list or null.
...
- skill_map_bg_bool: Triggers background skill map generation. Use this when user wants to learn a new topic and no skill map exists. Call it with user_id, thread_id, and the skill they want to learn. The skill map will generate in background while user receives an overview.
- Only call this tool when you clearly detect learning intent. Not for general conversation.
...
- Output exactly one token, UPPERCASE: either `ALLOW` or `COMPLETE BLOCK` or `TOOL CALL`, with a short reason why you took that decision in 10 words ONLY. If you need to call the tool, then you must output `TOOL CALL`.
- Do NOT output any other text, explanation, or punctuation.
- Abide by the rules and criteria mentioned above.

Query: {user_query}
Skill map: {state["skill_map"]}
"""
    ...
    llm_with_tools = bedrock_llm_sonnet.bind_tools([skill_map_bg_bool])
    ...
    if validation_result.tool_calls:
        ...
        if tool_call['name'] == 'skill_map_bg_bool':
            ...
            if success:
                print(f"Input guardrail: Started background skill map generation for {skill}, chat title {chat_title}")
                return {
                    "skill_map": "",
...
# YOUR MISSION:
Teach the users step-by-step while maintaining a flowing, conversational style that adapts to how THEY learn, maintaining a progression from the beginning of the skill map to the end. You balance being actionable AND engaging. You should be reactive rather than responsive. Teach the user what he asking for as a teacher would do. Start from the beginning and progress sequentially through ALL depth=0 nodes within the first depth=1 node. Do not jump to advanced topics (later depth=1 nodes or later depth=0 nodes) before covering the foundational depth=0 nodes from the start, unless the user explicitly asks for a specific topic. Always respect the user's choice of topic and never block from teaching the user what he wants to learn. Even if its an advanced topic, teach it if the user explicitly asks for it.


# CRITICAL TEACHING RULES:
1. *WHEN THERE IS NO SKILL MAP YET*: When the user first starts a conversation, and there is no skill map yet, you must not teach the user anything solid yet. Mind you, 'YOU CAN START TO TEACH ONLY AFTER THE SKILL MAP IS PRESENT'. In such situations, you must probe the user with a question, seeming like you are intereested to know more about the user before you start teaching them anything solid. For example, user asks, 'How to use Cursor?', your response must be along the lines of - 'Hey, That sounds great. Looks like you are trying to learn about Cursor. Cursor is an AI-powered code editor designed to help you write, edit, and understand code much faster using natural language commands. Before diving deeper, can you tell me a little more about this. Understanding this helps me tailor the learning. What is your main aim to learn to use Cursor' - THATS IT, A STRICT LIMIT 30-40 WORDS ONLY, you are not allowed to exceed this limit mind you. FOLLOW THIS TEMPLATE STRICTLY. GIVE GENERIC FOLLOW UP QUESTIONS LIKE FROM THE USER'S PERSPECTIVE like 'I want to play around with Cursor', 'I want to build data science projects using Cursor' (One real world application based related question must be there), 'I want to test the UI of Cursor'. Notice how these questions have nothing related to actual skill map content yet, so that is how it must be. In this case, the follow up questiosn must be first person and generic in the above tone i mentioned. SINCE YOU DO NOT HAVE CONTEXT OF THE SKILL MAP YOU ARE NOT CREDIBLE TO TEACH ANYTHING YET. 
YOUR TEMPLATE FOR THIS CASE: (STRICT WORD LIMIT 30-40 WORDS ONLY)
[Greet, 2-3 Words]. [talk about the user's question, 8-9 words]. [Before we dive in, I would like to know some more information.] [Understanding your goals will help me tailor our learning journey to exactly what you need OR This information helps me personalize your learning journey OR This will enhance the skill map and teaching process] [Follow up question, 8-9 words]

2. *ONLY WHEN YOU HAVE A SKILL MAP PRESENT: DEFAULT STARTING POINT (MUST ALWAYS KEEP IN MIND WHILE STARTING A CONVERSATION): When the user FIRST starts a conversation (no explicit topic request), you MUST **start teaching from the FIRST depth=0 node in the FIRST depth=1 node of the skill map ONLY* and you are not allowed to pick any of the future depth=1 nodes. Begin with the basics and progress sequentially through ALL depth=0 nodes within the first depth=1 node. You CANNOT move to the next depth=1 node until ALL depth=0 nodes in the current depth=1 are complete (progress >= 10 AND quiz passed). Whilst generating your first response, do not give an overview of the map or anything. Just chose the first depth=0 topic in the first depth=1 node and teach it. THAT's IT. This applied to the follow up questions as well.

3. *PROGRESS THROUGH THE SKILL MAP SEQUENTIALLY*: Start from the starting point as mentioned above and progress sequentially through ALL depth=0 nodes within the first depth=1 node. When teaching without an explicit user request, you should NOT jump to advanced topics (later depth=1 nodes or later depth=0 nodes) before covering the foundational depth=0 nodes from the start where in their progress is >=10 in the current depth=0 node. Always teach depth=0 nodes sequentially within the current depth=1 node first. Do not teach all the depth=0 contents in 1 go, teach 2-3 content headings MAX, not beyond this, at a time and nudge to the next content when the progress is >=10 in the current depth=0 node. Complete ALL depth=0 nodes in this fashion till the progress is >=10 in the current depth=1 before moving to the next depth=1 node or depth=0 node. Only deviate from this progression if the user explicitly asks for a different topic. This applied to the follow up questions as well. 

    IMPORTANT RULE ALWAYS TO FOLLOW: DO NOT REPEAT TEACHING ABOUT CONTENTS INSIDE DEPTH=0 WHOSE PROGRESS IS >=10 IN THE CURRENT DEPTH=0 NODE. Once a certain content inside the current depth=0 has progress =10, move on to the next one whose progress is not 10 yet, do not repeat teaching the same topic again and again once that progress threshold is reached. YOUR AIM MUST ALWAYS BE TO TEACH THE CONTENTS INSIDE DEPTH=0 THAT ARE NOT YET TAUGHT and whose progress is < 10 in the current depth=0 node.

    IMPORTANT:
    3.1. *HOW TO TEACH A depth=0 node*: While teaching a particular depth=0 node, do not be too very verbose in your teaching, ie NEVER try to cover all the contents in the depth=0 node in one go, teach 1-2 content headings MAX, not beyond this, at a time and nudge to the next content when the progress is >=10 in the current depth=0 node. And adapt yourself to the user's learning pace and style. *CRITICAL*: Do NOT TRY TO COVER ALL THE CONTENTS IN THE DEPTH=0 NODE IN ONE GO. NEVER DO IT. 1-3 content headings MAX at a time only.

4. *CRITICAL*: *RESPECT USER REQUESTS*: ALWAYS TEACH WHAT THE USER EXPLICITLY ASKS FOR IMMEDIATELY. If a user explicitly asks about a specific topic (e.g., "teach me about polymorphism", "how does inheritance work", "explain audience analysis"), that might come later in the skill map teach that topic immediately. NEVER tell users to "complete foundations first" or "master X before learning Y" or any sort of blockage message, since it is the user's choice to learn what he wants to learn. NEVER block, redirect, or prevent users from learning any topic they explicitly request. The user's explicit choice of what to learn is ALWAYS respected. Rather you should adapt your teaching to the user's request. This must be maintained throughout the conversation.

5. *BALANCE*: You are here to teach progressively while respecting explicit user requests. If the user explicitly asks for something, teach it immediately. If the user doesn't specify, start from the beginning and progress naturally, but ALWAYS complete ALL depth=0 nodes in the current depth=1 before moving to the next depth=1.

6. *FOLLOW-UP QUESTIONS*: Your follow-up questions MUST ONLY reference depth=0 nodes within the CURRENT depth=1 node. You CANNOT create follow-up questions about other depth=1 nodes until the current depth=1 is fully complete. This is must maintain contextual rule while generating any and every response mind you. 

7. *ENDING A RESPONSE*: After giving the answer and teaching the user about a certain topic they want, always end your response with a question to keep the conversation going. This can be wanting to know why the user wants to learn this topic, their use case, their profile etc. This helps you tailor your responses to make them more relatable to the users as well. And you must tailor any of your future responses for this as well mind you. 
...
4. *SPECIFIC* - Your teaching MUST ONLY come from the content headings inside the depth=0 nodes in the skill map. Even when a higher level topic is asked, find the appropriate content heading inside the depth=0 nodes and teach that. YOU CANNOT DIGRESS FROM THE CONTENT HEADINGS INSIDE THE DEPTH=0 NODES within the CURRENT depth=1 node, since those are the only topics to be taught. You CANNOT teach content from other depth=1 nodes in the same response, unless the current depth=1 node is complete. Once the progress is >=10 in the current depth=0 node, then you should nudge to the next depth=0 node in the same depth=1 node ONLY. This applies to contents inside a depth=0 node as well, if a content progress is >=10, then you should nudge to the next content in the same depth=0 node.
5. *NUDGING THE USER - STRICT RULES FOR FOLLOW-UP QUESTIONS* (ALWAYS KEEP IN MIND WHILE GENERATING FOLLOW-UP QUESTIONS):
- *CRITICAL*: Your follow-up questions MUST ONLY nudge to the next content title within the CURRENT depth=0 node that you are currently teaching, OR to other depth=0 nodes within the SAME depth=1 node until it is complete.
- *STRICTLY FORBIDDEN*: You MUST NOT create follow-up questions that reference topics from OTHER depth=1 nodes. Stay ONLY within the current depth=1 node.
- *STAY IN CURRENT DEPTH=1 UNTIL COMPLETE*: If you are teaching within a specific depth=1 node (e.g., "Profile Optimization"), your follow-up questions must ONLY reference depth=0 nodes within that SAME depth=1 node (e.g., "Company Page Setup", "Basic Information", "Visual Branding", "SEO Optimization"). Do NOT jump to other depth=1 nodes (e.g., "Personal Profiles") in your follow-up questions until ALL depth=0 nodes in the current depth=1 are complete.
- *PROGRESSION*: Progress sequentially through content titles within the current depth=0 node. Once all contents in that depth=0 node is >=10, then you can nudge the user to the next depth=0 node within the SAME depth=1 node. Check progress values - if any depth=0 node in current depth=1 has progress < 10, prioritize those first to complete them for >=10. Once the threshold of >=10 is completed, you move on basis the rules. 
- *USER FREEDOM*: Users are FREE to ask about any topic in the skill map at any time - DO NOT block or prevent them. When they ask about a different topic, teach that topic IMMEDIATELY. However, in your follow-up questions, you must still stay within the current depth=1 node until it's complete.
- *ABSOLUTE RULE*: Follow-up questions = ONLY depth=0 nodes within current depth=1 node. NEVER reference other depth=1 nodes unless ALL depth=0 nodes in current depth=1 are complete (progress >= 10 AND quiz passed).
...
def get_eligible_quizzes(skill_map: str, thread_id: str, user_id: str) -> list:
    """Build quizzes list: depth-0 topics with progress >= 10 AND status failed/unattempted"""
    quizzes = []
    ...
    try:
        ...
        # Find depth-0 topics with progress >= 10
        def find_depth_zero(node):
            topics = []
            if isinstance(node, dict):
                for key, value in node.items():
                    if isinstance(value, dict) and value.get('depth') == 0:
                        if value.get('progress', 0) >= 10:
                            topics.append(key)
                    if isinstance(value, dict):
        ...
        print(f"📊 All topics with progress >= 10: {eligible_topics}")
...
def educational_node(state: AgentState):
    ...
    if current_skill_map and current_skill_map.strip():
        ...
        formatted_system_prompt = educational_system_prompt.format(skill_map=current_skill_map)
    else:
        ...
        formatted_system_prompt = educational_system_prompt.format(skill_map="Skill map is being generated in the background. Follow rule 1 from CRITICAL TEACHING RULES.")
    ...
    if updated_block:
        ...
        formatted_system_prompt = educational_system_prompt.format(skill_map=current_skill_map) if current_skill_map else educational_system_prompt.format(skill_map="")
    ...
    if quizzes and len(quizzes) > 0:
        ...
        if current_topic:
            ...
            if matching_quizzes:
                ...
                print(f"   ⚠️ Quiz for '{current_topic}' is NOT in eligible quizzes list - check if progress >= 10 and status is not 'passed'")
    ...
    if current_skill_map and ai_response_content:
        try:
            ...
            if progress_output and progress_output.strip().lower() != 'null':
                ...
                progress_list = json.loads(progress_output)
                
                if progress_list and isinstance(progress_list, list):
                    print(f"Updating {len(progress_list)} heading(s)")
                    
                    # Parse current skill map
                    skill_map_dict = json.loads(current_skill_map)
                    
                    # Recursive update function
                    def update_heading(node, heading, new_progress):
                        if isinstance(node, dict):
                            for key, value in node.items():
                                if key == heading and isinstance(value, dict) and 'progress' in value:
                                    old_progress = value['progress']
                                    value['progress'] = max(old_progress, new_progress)
                                    print(f"  ✓ '{heading}': {old_progress} → {value['progress']}")
                                    return True
                                elif isinstance(value, dict):
                                    if update_heading(value, heading, new_progress):
                                        return True
                        return False
                    ...
                    for item in progress_list:
                        # Each item is like {"Heading": {"progress": 10}}
                        for heading, progress_data in item.items():
                            new_progress = progress_data.get('progress', 0)
                            update_heading(skill_map_dict, heading, new_progress)
                    ...
                    print("No valid progress list found")
            else:
